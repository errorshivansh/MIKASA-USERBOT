...
@bot.on(mikasa_cmd(pattern="wikipedia (.*)"))
...
@bot.on(mikasa_cmd(pattern="watch (.*)"))
@bot.on(sudo_cmd(pattern="watch (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    query = event.pattern_match.group(1)
    mikasa = await eor(event, "Finding Sites...")
...
@bot.on(mikasa_cmd(pattern="img (.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="img (.*)", allow_sudo=True))
async def img(event):
    sim = event.pattern_match.group(1)
    if not sim:
        return await eod(event, "`Give something to search...`")
    mikasa = await eor(event, f"Searching for  `{sim}`...")
    if "-" in sim:
        try:
            lim = int(sim.split(";")[1])
            sim = sim.split(";")[0]
...
@bot.on(mikasa_cmd(pattern="reverse"))
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        mikasa = await eor(event, "Pre Processing Media")
        ...
        if previous_message.media:
            ...
            the_location = google_rs_response.headers.get("Location")
        await mikasa.edit("Found Google Result. Processing results...")
        headers = {
...
More Info: Open this <a href="{the_location}">Link</a> in {ms} seconds""".format(
    ...
    await mikasa.edit(OUTPUT_STR, parse_mode="HTML", link_preview=False)


@bot.on(mikasa_cmd(pattern="gps ?(.*)"))
...
async def gps(event):
    ...
    geolocator = Nominatim(user_agent="MikasaBot")
