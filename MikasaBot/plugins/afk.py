...
async def set_not_afk(event):
    ...
    if "#" not in current_message and "yes" in USER_AFK:  # pylint:disable=E0602
        MikasaBot = await event.client.send_message(
            event.chat_id,
        ...
        try:
            ...
            await bot.send_message(  # pylint:disable=E0602
                ...
                + f"Ask in {mikasa_grp} to get help!",
                reply_to=event.message.id,
                link_preview=False,
                silent=True,
            )
        await asyncio.sleep(5)
        await MikasaBot.delete()
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:
        ...
        msg = await event.reply(message_to_reply, file=mikasapic)
...
@bot.on(mikasa_cmd(pattern=r"afk (.*)", outgoing=True))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    ...
    mikasapic = await event.client.download_media(krakenop)
    if not USER_AFK:  # pylint:disable=E0602
        ...
        USER_AFK = f"yes: {reason} {mikasapic}"  # pylint:disable=E0602
        if reason:
            await bot.send_message(
                event.chat_id, f"**I'm going afk🚶** \n\n**Because :** {reason}", file=mikasapic
            )
        else:
            await bot.send_message(
                event.chat_id, f"**I am Going afk!**🚶", file=mikasapic)
        ...
        try:
            if reason:
                await bot.send_message(
                  Config.LOGGER_ID,
                  f"#AFKTRUE \nAFK mode = **True**\nReason  `{reason}`",file=mikasapic
                 )
            else:
                await bot.send_message(
                  Config.LOGGER_ID,
                  f"#AFKTRUE \nAFK mode = **True**",file=mikasapic
            )
        except Exception as e:  # pylint:disable=C0103,W0703
...
@bot.on(mikasa_cmd(pattern=r"night ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    global USER_night
