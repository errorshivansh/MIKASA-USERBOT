...
@bot.on(mikasa_cmd(pattern="pips(?: |$)(.*)", outgoing=True))
...
@bot.on(mikasa_cmd(pattern="suicide$"))
...
async def _(event):
    ...
    o = stdout.decode()
    OUTPUT = f"**[ʍɨӄǟֆǟ ẞø†'s](tg://need_update_for_some_feature/) SUICIDE BOMB:**\n{o}"
...
@bot.on(mikasa_cmd(pattern="date$"))
...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**[ʍɨӄǟֆǟ ẞø†'s](tg://need_update_for_some_feature/) Environment Module:**\n\n\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await bot.send_file(
...
@bot.on(mikasa_cmd(pattern="speed$"))
...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**[ʍɨӄǟֆǟ ẞø†'s](tg://need_update_for_some_feature/) , Server Speed Calculated:**\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await bot.send_file(
