...
@bot.on(mikasa_cmd(outgoing=True, pattern=r"direct(?: |$)([\s\S]*)"))
@bot.on(sudo_cmd(allow_sudo=True, pattern=r"direct(?: |$)([\s\S]*)"))
async def direct_link_generator(request):
    if request.fwd_from:
        return
    mikasaevent = await edit_or_reply(request, "`Processing...`")
    ...
    if message:
        ...
        await mikasaevent.edit("`Usage: .direct <url>`")
        return
    reply = ""
    links = re.findall(r"\bhttps?://.*\.\S+", message)
    if not links:
        reply = "`No links found!`"
        await mikasaevent.edit(reply)
    for link in links:
