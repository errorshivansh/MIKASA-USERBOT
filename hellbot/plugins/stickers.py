...
@bot.on(mikasa_cmd(outgoing=True, pattern="kang"))
...
async def kang(args):
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            mikasa = await eor(args, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
    ...
    if photo:
        ...
        packnick = (
            ...
            if MikasaBot
            else f"@{user.username}'s MikasaBot Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
        ...
        if (
            ...
            await mikasa.edit("`Preparing a new pack....`")
            async with bot.conversation("Stickers") as conv:
                await conv.send_message(cmd)
                await conv.get_response()
                # Ensure user doesn't get spamming notifications
...
@bot.on(mikasa_cmd(pattern=r"delst ?(.*)", outgoing=True))
...
@bot.on(mikasa_cmd(pattern=r"editst ?(.*)", outgoing=True))
...
async def _(event):
    ...
    if not event.reply_to_msg_id:
        ...
        return
    reply_message = await event.get_reply_message()
    mikasa = event.pattern_match.group(1)
    chat = "@Stickers"
    reply_message.sender
    if reply_message.sender.bot:
        await edit_or_reply(event, "`Reply to actual user's message.`")
    ...
    if mikasa == "":
        ...
        async with bot.conversation(chat) as conv:
            try:
                ...
                await conv.get_response()
                await asyncio.sleep(2)
                await conv.send_message(f"{mikasa}")
                response = await response
            except YouBlockedUserError:
                await event.reply("Please unblock @Stickers and try again")
                return
...
@bot.on(mikasa_cmd(pattern="text (.*)"))
...
@bot.on(mikasa_cmd(pattern="waifu(?: |$)(.*)", outgoing=True))
