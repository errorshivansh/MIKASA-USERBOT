...
@bot.on(mikasa_cmd(pattern="stoi$"))
@bot.on(sudo_cmd(pattern="stoi$", allow_sudo=True))
async def _(mikasa):
    if mikasa.fwd_from:
        return
    reply_to_id = mikasa.message.id
    if mikasa.reply_to_msg_id:
        reply_to_id = mikasa.reply_to_msg_id
    event = await eor(mikasa, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await mikasa.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await mikasa.client.send_file(
                event.chat_id,
...
@bot.on(mikasa_cmd(pattern="itos$"))
...
@bot.on(mikasa_cmd(pattern="ftoi$"))
...
@bot.on(mikasa_cmd(pattern="gif$"))
@bot.on(sudo_cmd(pattern="gif$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    mikasareply = await event.get_reply_message()
    if not mikasareply or not mikasareply.media or not mikasareply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if mikasareply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    reply_to_id = event.message
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    chat = "@tgstogifbot"
    mikasaevent = await edit_or_reply(event, "`Converting to gif ...`")
    async with bot.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await bot.send_file(chat, mikasareply.media)
            response = await conv.get_response()
            await bot.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await mikasaevent.edit("`This file is not supported`")
            mikasaresponse = response if response.media else await conv.get_response()
            await bot.send_read_acknowledge(conv.chat_id)
            mikasafile = Path(await event.client.download_media(mikasaresponse, "./temp/"))
            mikasagif = Path(await unzip(mikasafile))
            ...
            for files in (mikasagif, mikasafile):
                if files and os.path.exists(files):
                    os.remove(files)
        except YouBlockedUserError:
            await mikasaevent.edit("Unblock @tgstogifbot")
            return


@bot.on(mikasa_cmd(pattern="nfc ?(.*)"))
