...
@bot.on(mikasa_cmd(pattern=r"kickall ?(.*)"))
...
async def _(event):
    ...
    mikasa = await eor(event, "**Bleck Magik Started...**")
...
@bot.on(mikasa_cmd(pattern=r"banall ?(.*)"))
...
async def _(event):
    ...
    mikasa = await eor(event, "**Bleck Magik Begins..**")
    admins = await event.client.get_participants(
        event.chat_id, filter=ChannelParticipantsAdmins
    )
    admins_id = [i.id for i in admins]
...
@bot.on(mikasa_cmd(pattern=r"unbanall ?(.*)"))
...
@bot.on(mikasa_cmd(pattern="ikuck ?(.*)"))
...
async def _(event):
    ...
    q = 0
    r = 0
    mikasa = await edit_or_reply(event, "Searching Participant Lists.")
    async for i in bot.iter_participants(event.chat_id):
        ...
        if isinstance(i.status, UserStatusEmpty):
            ...
            if "y" in input_str:
                ...
                if not status:
                    await eod(mikasa, "I need admin priveleges to perform this action!")
                    e.append(str(e))
                    break
                else:
                    c = c + 1
...
Bots: {}
None: {}"""
        await mikasa.edit(required_string.format(c, p, d, y, m, w, o, q, r, b, n))
