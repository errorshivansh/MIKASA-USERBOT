...
@bot.on(mikasa_cmd(pattern="scan ?(.*)"))
...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    mikasaevent = await eor(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await eod(mikasaevent, "`Please unblock `@DrWebBot `and try again`")
            return
        if response.text.startswith("Forward"):
            await eod(mikasaevent,
...
@bot.on(mikasa_cmd(pattern=r"decode$", outgoing=True))
...
@bot.on(mikasa_cmd(pattern="barcode ?(.*)"))
@bot.on(sudo_cmd(pattern="barcode ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    mikasaevent = await eor(event, "...")
    ...
    try:
        ...
        await mikasaevent.edit(str(e))
        return
    end = datetime.datetime.now()
    ms = (end - start).seconds
    await mikasaevent.edit("Created BarCode in {} seconds🤓".format(ms))
    await asyncio.sleep(5)
    await mikasaevent.delete()


@bot.on(mikasa_cmd(pattern=r"makeqr(?: |$)([\s\S]*)", outgoing=True))
...
async def _(event):
    ...
    if len(input_sgra) == 3:
        ...
        await mikasa.edit(str(a))
    else:
        await eod(mikasa, f"SYNTAX: {hl}calendar YYYY.MM.DD")
    end = datetime.datetime.now()
    (end - start).seconds


@bot.on(mikasa_cmd(pattern="currency (.*)"))
...
@bot.on(mikasa_cmd(pattern="currencies$"))
...
@bot.on(mikasa_cmd(pattern="ifsc (.*)"))
...
@bot.on(mikasa_cmd(pattern="color (.*)"))
...
@bot.on(mikasa_cmd(pattern="xkcd ?(.*)"))
@bot.on(sudo_cmd(pattern="xkcd ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    mikasaevent = await eor(event, "`processiong...`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await mikasaevent.edit(output_str, link_preview=True)
    else:
        await eod(mikasaevent, "xkcd n.{} not found!".format(xkcd_id))

@bot.on(mikasa_cmd(pattern="dns (.*)", outgoing=True))
...
@bot.on(mikasa_cmd(pattern="url (.*)", outgoing=True))
...
@bot.on(mikasa_cmd(pattern="unshort (.*)", outgoing=True))
...
CmdHelp("tools").add_command(
  ...
  "currency", "<amount> <from> <to>", "Currency converter for MikasaBot", ".currency 10 usd inr"
).add_command(
