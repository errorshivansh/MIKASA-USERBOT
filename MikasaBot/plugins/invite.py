...
@bot.on(mikasa_cmd(pattern=r"inviteall ?(.*)"))
...
async def get_users(event):
    ...
    if event.is_private:
        return await mikasa.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"

    await mikasa.edit("**INVITING USERS !!**")
...
@bot.on(mikasa_cmd(pattern=r"add ?(.*)"))
