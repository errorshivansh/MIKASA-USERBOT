...
@bot.on(mikasa_cmd(pattern=r"purge", outgoing=True))
...
@bot.on(mikasa_cmd(pattern=r"purgeme", outgoing=True))
...
@bot.on(mikasa_cmd(pattern=r"sd", outgoing=True))
...
CmdHelp("purger").add_command(
  ...
  "sd", "<time> <text>", "Sends a self destruct text. Fill time in secs", "sd 10 mikasao"
).add_command(
