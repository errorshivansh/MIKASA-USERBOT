...
@bot.on(mikasa_cmd(pattern="recognize ?(.*)", outgoing=True))
...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    mikasa = await eor(event, "recognizeing this media")
    async with event.client.conversation(chat) as conv:
        ...
        if second.text.startswith("See next message."):
            ...
            mikasa = third.message.message
            await eor(event, mikasa)
...
@bot.on(mikasa_cmd(pattern="info ?(.*)", outgoing=True))
...
<b>⚡ <a href='https://t.me/MIKASA_BOT_OP'>From DataBase of MikasaBot</a> ⚡ </b>
""".format(
        user_id,
        user_id,
        first_name,
...
@bot.on(mikasa_cmd(pattern="chatinfo(?: |$)(.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="chatinfo(?: |$)(.*)", allow_sudo=True))
async def info(event):
    if event.fwd_from:
        return
    mikasa = await eor(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event)
    caption = await fetch_info(chat, event)
    try:
        await mikasa.edit(caption, parse_mode="html")
    except Exception as e:
        print("Exception:", e)
        await eod(mikasa, "`An unexpected error has occurred.`")
    return
...
@bot.on(mikasa_cmd(pattern=r"users ?(.*)", outgoing=True))
...
@bot.on(mikasa_cmd(pattern="admins ?(.*)"))
...
@bot.on(mikasa_cmd(pattern="bots ?(.*)"))
...
@bot.on(mikasa_cmd(pattern="id$"))
@bot.on(sudo_cmd(pattern="id$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    mikasa = await eor(event, "Fetching Ids...")
    if event.reply_to_msg_id:
        ...
        await mikasa.edit("🔸 **Current Chat ID:** `{}`".format(str(event.chat_id)))
